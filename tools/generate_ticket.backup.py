...
def main():
    ...
    ap.add_argument("--model", required=False, help="Optional logistic model JSON (a,b). If missing, use gentle logistic fallback")
    ...
    # Try to load model; if missing/invalid -> gentle fallback
    use_model = False
    ...
    if not use_model:
        board["win_prob"] = sigmoid(0.35 * board["Edge"])  # gentle fallback

    board["void_prob"] = float(args.void_prob)
    board[["Prop","Player","win_prob","void_prob"]].to_csv(args.out, index=False)
    print(f"Wrote ticket -> {args.out} (rows={len(board)}) | model_used={use_model}")
